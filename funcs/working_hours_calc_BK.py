...
def billable_hours_calc(row):
    if row['day'] in ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat'] and row['is_holiday'] == 'N' :
        # Shift Malam
        if row['jam_mulai'].hour >= 18 or row['jam_mulai'].hour < 6:
            # 7 jam pertama 1x
        ...
        # 1 jam berikutnya 1.5x
            ...
            return 5 + (row['total_working_hours'] - 6) * 1.5
        # jam berikutnya 2x
        else:
...
def working_hours_calc(attendance_data_df,holidays_date_df): # attendance_data_df, start_date, end_date
